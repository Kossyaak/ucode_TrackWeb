...
function init() {
	...
	labels["logo"] = new Label({x: 0.5, y: 0.3}, "Anonymous game", 100, true, false, false, "NoirPro-Light");
	labels["play"] = new Label({x: 0.1, y: 0.1}, "▶", 50, true, true, false, labelFont, enterQueue);
	labels["searching"] = new Label({x: 0.2, y: 0.1}, "Searching   ", 40, false, false, false, labelFont);
	labels["result"] = new Label({x: 0.5, y: 0.3}, "", 100, false, false, false, labelFont);
	labels["rematch"] = new Label({x: 0.5, y: 0.62}, "Rematch", 100, false, false, false, labelFont, requestRematch);
	labels["waiting"] = new Label({x: 0.5, y: 0.62}, "Waiting   ", 70, false, false, false, labelFont);
	labels["main menu"] = new Label({x: 0.5, y: 0.78}, "Main Menu", 70, false, false, false, labelFont, exitMatch);
...
function drawCard(card, position, scale) {
	...
	if(types[card.type] == "Fire")
	{
		ctx.drawImage(fireAnonymous, position.x, position.y, cardWidth * scale, cardHeight * scale);
	}
	if(types[card.type] == "Water")
	{
		ctx.drawImage(waterAnonymous, position.x, position.y, cardWidth * scale, cardHeight * scale);
	}
	if(types[card.type] == "Ice")
	{
		ctx.drawImage(icedAnonymous, position.x, position.y, cardWidth * scale, cardHeight * scale);
	}
	ctx.fillStyle = typeColors[card.type];
	ctx.font = "bold " + (36 * scale * r) + "px chinese_takeaway";
	ctx.fillText(card.power, position.x + cardWidth * scale / 1.22, position.y + cardHeight * scale * 0.17);
...
function drawLabel(label) {
	...
	let shadowDistance = label.size / 50;
	if (!label.disabled)
	{
		ctx.fillStyle = "white";
...
let clickCursor = false,
	...
	aspect = 16 / 10,
	labels = [],
	labelFont = "NoirPro-Light";

let typeColors = ["#e03b0d", "#1260E6", "#74D5F2"];
let types = ["Fire", "Water", "Ice"];
let colors = {"yellow": "#fdee00", "orange": "#ffb235", "green": "#52a546", "blue": "#246acd", "red": "#e02929", "purple": "#9738af"};
let fireAnonymous = new Image(100, 200)
fireAnonymous.src = './images/F.png';
let waterAnonymous = new Image(100, 200)
waterAnonymous.src = './images/W.png';
let icedAnonymous = new Image(100, 200)
icedAnonymous.src = './images/I.png';
let cards = [fireAnonymous, waterAnonymous, icedAnonymous];
init();
animate();
