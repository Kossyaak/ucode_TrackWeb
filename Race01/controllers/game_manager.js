...
let powers = [10, 8, 7, 6, 5, 5, 4, 3, 3, 2];
let colors = ["yellow", "orange", "green", "blue", "red", "purple"];
...
function cursReady(match) {
	if (logFull) console.log("%s(%j)", arguments.callee.name, Array.prototype.slice.call(arguments).sort());
	let isReady = (match.players[0].cur && match.players[1].cur);
	return isReady;
}
...
function processRound(match, tied, winner) {
	if (logFull) console.log("%s(%j)", arguments.callee.name, Array.prototype.slice.call(arguments).sort());
	let loser = match.players[match.players[0] !== winner ? 0 : 1];
	if (!tied) {
...
function checkForSet(player) {
	...
	for (let i = 0; i < player.points.length; i++) {
		let setColors = [];
		for (let j = 0; j < player.points[i].length; j++) {
	...
	for (let i = 0; i < player.points[0].length; i++) {
		for (let j = 0; j < player.points[1].length; j++) {
			for (let k = 0; k < player.points[2].length; k++) {
...
function leaveMatch(socket) {
	...
	if (match) {
		if (!match.isOver) {
			let winner = match.players[match.players[0].socket.id !== socket.id ? 0 : 1];
